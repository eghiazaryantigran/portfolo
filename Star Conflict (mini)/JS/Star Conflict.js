var imgstr = ['img/1.jpg','img/2.png','img/3.png','img/4.png','img/5.png'];
...
let Humman_gan =new Audio();

Humman_gan.src="audio/Humman_gan.mp3"

let ufo_gan =new Audio();

ufo_gan.src="audio/ufo_gan.mp3"
...
function cls_you_ebul(j)
{
	...
	if ( (x2+imgx[4]>x1)&&(x1+imgx[1]>x2)&&(y2+imgy[4]>y1)&&(y1+imgy[1]>y2) )
	{
		endgame = 1;
		end1.play();
		alert('Դուք հավաքել եք '+score+' միավոր. Թարմացնել էջը նորից սկսելու համար 😂');
		return 1;
	}
...
function draw()
{
	...
	ctx.drawImage(img[1],youx,youy);
	
	for (var i=0;i<buls.length;i++)
	{
		ctx.drawImage(img[2],buls[i].x,buls[i].y);
	}
	
	for (var i=0;i<enemies.length;i++)
	{
		ctx.drawImage(img[3],enemies[i].x,enemies[i].y);
	}
	
	for (var i=0;i<ebuls.length;i++)
	{
		ctx.drawImage(img[4],ebuls[i].x,ebuls[i].y);
	}	
